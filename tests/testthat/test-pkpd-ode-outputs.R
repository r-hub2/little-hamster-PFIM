...
test_that("bolus PK analytic + PD ODE remaps RespPK library notation", {
  modelFromLibrary = list(
    PKModel = "Linear1BolusSingleDose_ClV",
    PDModel = "TurnoverkoutEmax_RinEmaxCC50koutE"
  )
  ...
  administration = Administration( outcome = "Cc", timeDose = 0, dose = 10 )
  arm1 = Arm(
    name = "BrasTest",
    size = 40,
    administrations = list( administration ),
    samplingTimes = list(
      SamplingTimes( outcome = "RespPK", samplings = c( 0, 1, 2 ) ),
      SamplingTimes( outcome = "RespPD", samplings = c( 0, 2, 10 ) )
    ),
    initialConditions = list( Cc = 0, E = 0 )
  )
  evaluation = Evaluation(
    ...
    modelError = modelError,
    outputs = list( "RespPK" = "Cc", "RespPD" = "E" ),
    designs = list( Design( name = "design1", arms = list( arm1 ) ) ),
    fimType = "population",
    odeSolverParameters = list( atol = 1e-4, rtol = 1e-4 )
  )

  eq = defineModelEquationsFromLibraryOfModel( evaluation )
  eq_text = paste( unlist( eq, use.names = TRUE ), collapse = " " )
  expect_false( any( grepl( "(?<!dose_)(?<!Tinf_)RespPK", eq_text, perl = TRUE ) ) )
})

test_that("infusion PK analytic + PD ODE remaps library notation to user compartments", {
  modelFromLibrary = list(
  ...
  administration = Administration(
    outcome = "Cc", timeDose = 0, dose = 10, Tinf = 2
  )
  arm1 = Arm(
    name = "BrasTest",
    size = 40,
    administrations = list( administration ),
    samplingTimes = list(
      SamplingTimes( outcome = "RespPK", samplings = c( 0, 1, 2, 5 ) ),
      SamplingTimes( outcome = "RespPD", samplings = c( 0, 2, 10 ) )
    ),
    initialConditions = list( Cc = 0, E = 0 )
  )
  ...
  eq = defineModelEquationsFromLibraryOfModel( evaluation )
  eq_text = paste( unlist( eq, use.names = TRUE ), collapse = " " )
  derivNames = names( unlist( eq, recursive = TRUE, use.names = TRUE ) )

  expect_true( any( grepl( "Deriv_Cc", derivNames, fixed = TRUE ) ) )
  expect_true( any( grepl( "Deriv_E",  derivNames, fixed = TRUE ) ) )
  expect_false( any( grepl( "(?<!dose_)(?<!Tinf_)RespPK", eq_text, perl = TRUE ) ) )
})
