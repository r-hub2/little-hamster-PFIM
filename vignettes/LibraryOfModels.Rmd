...
Six parameters are common to one- and two-compartment models:
...
```{r class.source= ".boxModel", eval= FALSE}
MichaelisMenten1InfusionSingleDose_VmKmV
```
...
*No separate library preset for multi-dose one-compartment MM infusion; use repeated single administrations or custom ODE equations.*
...
The central volume is $V_1$ (catalogue suffix `V1V2`), not $V$.
...
C_1\left(t_{D}\right)=&{\frac{D}{V_1}}\\[0.05cm]
\end{cases}\\[0.15cm]
&\frac{dC_{1}}{dt}=-\frac{{V_m}\times C_1}{K_m+C_1}-k_{12}C_1+\frac{ k_{21 }V_2}{V_1}C_2\\[0.2cm]
&\frac{dC_{2}}{dt}=\frac{ k_{12 }V_1}{V_2}C_1-k_{21}C_2	\\
\end{aligned}
\end {equation}$$
...
C_2\left(t\right)=&0 \text{	for $t\leq t_D$}\\[0.05cm]
\end{cases}\\[0.15cm]
&\frac{dC_{1}}{dt}=-\frac{{V_m}\times C_1}{K_m+C_1}-k_{12}C_1+\frac{ k_{21 }V_2}{V_1}C_2+input\\[0.2cm]
&\frac{dC_{2}}{dt}=\frac{ k_{12 }V_1}{V_2}C_1-k_{21}C_2\\[0.2cm]
&input\left(t\right)=\begin{cases}
{\frac{D}{Tinf}\frac{1}{V_1}} &\text{if $0\leq t-t_{D}\leq Tinf$}\\[0.05cm]
0 &\text{if not.}
\end{cases}
\end{aligned}
\end {equation}$$
...
&input\left(t\right)=
\begin{cases}
{\frac{D_{i}}{Tinf_{i}}\frac{1}{V_1}} &\text{if $0\leq t-t_{D_{i}}\leq Tinf_{i}$,}\\[0.05cm]
0 &\text{if not.}
\end{cases}
\end{aligned}
\end {equation}$$
...
&\frac{dC_{1}}{dt}=-\frac{{V_m}\times C_1}{K_m+C_1}-k_{12}C_1+\frac{ k_{21}V_2}{V_1}C_2+input\\[0.2cm]
&\frac{dC_{2}}{dt}=\frac{ k_{12}V_1}{V_2}C_1-k_{21}C_2\\[0.2cm]
&input\left(t\right)=\frac{D}{V_1}k_ae^{-k_a\left(t-t_D\right)}
\end{aligned}
\end {equation}$$
...
&\frac{dC_{1}}{dt}=-\frac{{V_m}\times C_1}{K_m+C_1}-k_{12}C_1+\frac{ k_{21}V_2}{V_1}C_2+input\\[0.2cm]
&\frac{dC_{2}}{dt}=\frac{ k_{12}V_1}{V_2}C_1-k_{21}C_2\\[0.2cm]
&input\left(t\right)=\sum^{n}_{i=1}\frac{D_i}{V_1}k_ae^{-k_a\left(t-t_{D_{i}}\right)}
\end{aligned}
\end {equation}	$$	
...
+ full $I_{max}$ model
$$\begin{equation}
A\left(t\right)=1-\frac{C\left(t\right)}{C\left(t\right)+C_{50}}
\end{equation}$$	

```{r class.source= ".boxModel", eval= FALSE}
ImmediateDrugFullImax_S0C50
```

+ sigmoïd full $I_{max}$ model
$$\begin{equation}
A\left(t\right)=1-\frac{C\left(t\right)^{\gamma}}{C\left(t\right)^{\gamma}+C_{50}^{\gamma}}
\end{equation}$$

```{r class.source= ".boxModel", eval= FALSE}
ImmediateDrugSigmoidFullImax_S0C50gamma
```

### Baseline/disease models {#baselinediseasemodel}

+ null baseline (no extra disease term: \(E=A\))

$$\begin{equation}
S\left(t\right)=0
\end{equation}$$

There is no library entry for \(S=0\); combine a drug-action model alone.
